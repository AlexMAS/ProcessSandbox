...
Стандартный класс `System.Diagnostics.Process` имеет множество неочевидных тонкостей в использовании. Класс `ProcessSandbox`
устраняет этот недостаток, скрывая всевозможные сложности. Ниже приведен пример кода, который запускает некоторый процесс
с ограничениями использования CPU и памяти, выводя статистику исполнения в консоль.
...
В результате выполнения сценария формируется четыре файла, которые сохраняются в каталог [`out`](out):
...
Для запуска тестов используется скрипт [`test.sh`](test.sh), который находит все тестовые случаи и выполнит каждый в
отдельном Docker-контейнере.
